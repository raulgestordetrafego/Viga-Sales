#!/usr/bin/env python3
"""
Ferramenta de busca inteligente no Cerebro do Chief (CEO).
Uso: python3 scripts/query_chief_brain.py "<pergunta>"
     python3 scripts/query_chief_brain.py --keywords "precificacao vendas"
     python3 scripts/query_chief_brain.py --topic fundamento
     python3 scripts/query_chief_brain.py --list
"""
...
BRAIN_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "brain", "chief")
...
def get_topic(topic_name):
    ...
    if topic_name not in index["topicos"]:
        print(json.dumps({"error": f"Topico '{topic_name}' nao encontrado", "disponiveis": list(index["topicos"].keys())}, ensure_ascii=False))
        sys.exit(1)

    arquivo_rel = index["topicos"][topic_name]["arquivo"]
    if arquivo_rel.startswith("brain/"):
        arquivo_rel = arquivo_rel[6:]
    if arquivo_rel.startswith("chief/"):
        arquivo_rel = arquivo_rel[6:]
    arquivo = os.path.join(BRAIN_DIR, arquivo_rel)
    if not os.path.exists(arquivo):
        print(json.dumps({"error": f"Arquivo {arquivo} nao encontrado"}, ensure_ascii=False))
        sys.exit(1)
...
def main():
    if len(sys.argv) < 2:
        print(json.dumps({"error": "Uso: query_chief_brain.py '<pergunta>' | --list | --topic <nome> | --keywords <termos>"}, ensure_ascii=False))
        sys.exit(1)
    ...
    if arg == "--list":
        ...
        if len(sys.argv) < 3:
            print(json.dumps({"error": "--topic requer nome do topico"}, ensure_ascii=False))
            sys.exit(1)
