    ...
    try {
      ...
      for (const update of data.result || []) {
        ...
        if (lower === '/tarefas' || lower === 'tarefas') {
          ...
          if (!tasks.length) { await api.sendText(fromId, 'Nenhuma tarefa pendente.'); continue; }
        ...
        if (lower === '/cerebro' || lower === 'cerebro') {
          ...
          try {
            ...
            if (result.error || !result.matched_topics?.length) {
              await api.sendText(fromId, '🧠 Nao encontrei nada sobre isso.');
            } else {
          ...
          } catch (e) { await api.sendText(fromId, 'Erro ao consultar cerebro.'); }
          continue;
        }

        handleBossCommand(fromId, msg.text, name, api).catch(e => {
          console.error('[Telegram] Erro ao processar:', e.message);
          api.sendText(fromId, 'Erro ao processar.').catch(() => {});
        });
      }
    } catch (e) {
      console.error('[Telegram] Poll error:', e.message || e.code || 'desconhecido');
      await sleep(3000);
    }
  }
}
